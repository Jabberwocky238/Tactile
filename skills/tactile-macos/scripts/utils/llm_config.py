...
DEFAULT_LLM_MODEL = os.getenv("MACOS_APP_WORKFLOW_MODEL", "gpt-5.5")
LLM_TIMEOUT_SECONDS = float(os.getenv("MACOS_APP_WORKFLOW_LLM_TIMEOUT", "600"))
LLM_MAX_RETRIES = int(os.getenv("MACOS_APP_WORKFLOW_LLM_MAX_RETRIES", "3"))
LLM_RETRY_DELAY_SECONDS = float(os.getenv("MACOS_APP_WORKFLOW_LLM_RETRY_DELAY", "5"))
PROXY_ENV_KEYS = (
    "ALL_PROXY",
    "all_proxy",
    "HTTP_PROXY",
...
def _env_key(provider: str, suffix: str) -> str:
    ...
    return f"MACOS_APP_WORKFLOW_{normalized}_{suffix}"


def _client_config(provider: str | None) -> tuple[str, str | None]:
    api_key = None
    base_url = None
    if provider:
        api_key = os.getenv(_env_key(provider, "API_KEY"))
        base_url = os.getenv(_env_key(provider, "BASE_URL"))

    api_key = api_key or os.getenv("MACOS_APP_WORKFLOW_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
    base_url = base_url or os.getenv("MACOS_APP_WORKFLOW_OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL")
    if not api_key:
        raise RuntimeError(
            "missing LLM API key; set OPENAI_API_KEY or MACOS_APP_WORKFLOW_OPENAI_API_KEY"
        )
