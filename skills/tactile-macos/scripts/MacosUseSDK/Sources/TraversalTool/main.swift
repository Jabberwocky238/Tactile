...
if let flagIndex = arguments.firstIndex(of: "--no-activate") {
    activateApp = false
    arguments.remove(at: flagIndex)
...
guard let pidStr = pidString, let appPID = Int32(pidStr) else {
    fputs("usage: TraversalTool [--visible-only] [--no-activate] <PID>\n", stderr)
    ...
    fputs("example (visible only without changing focus): TraversalTool --visible-only --no-activate 14154\n", stderr)
    exit(1)
}

// 2. Call the Library Function
do {
    fputs("info: calling traverseAccessibilityTree for pid \(appPID) (Visible Only: \(onlyVisible), Activate App: \(activateApp))...\n", stderr)
