...
第一版以 skill 的形式接入 Codex。在早期测试中，对于无障碍适配较好的 macOS 应用，accessibility-first 流程可以显著减少截图推理和坐标重试。随着执行经验被沉淀为可复用的策略、示例和工具约束，skill 也可以持续进化，并带来更稳定的任务效果；这种经验复用在许多自动化工作中已经被反复证明有效。
