...
## 在 Codex 中安装和使用 Skill

Tactile 的推荐用法是先安装成 Codex skill，然后在对话里让 Codex 通过 `$tactile-windows` 或 `$tactile-macos` 操作本机应用。

按运行 Codex 的系统选择对应 skill：

- Windows: `skills/tactile-windows`
- macOS: `skills/tactile-macos`

Windows skill 已经把 Windows UI Automation 运行时打包在 `skills/tactile-windows/vendor/WindowsUseSDK`，fresh clone 后不需要再单独准备 `WindowsUseSDK` checkout。
...
```txt
请从 https://github.com/yliust/Tactile 为我安装当前系统对应的 Tactile skill。
如果是 Windows，请安装 skills/tactile-windows，skill 名称为 tactile-windows。
如果是 macOS，请安装 skills/tactile-macos，skill 名称为 tactile-macos。
安装后请用这个 skill 观察一个简单应用，确认可用。
```

如果安装后当前会话没有刷新 skill 列表，重新打开一个 Codex 会话即可。
...
安装后，在 Codex 里明确要求使用 skill，而不是直接看截图：

```txt
请使用 $tactile-windows 列出 Windows 应用，打开 Calculator，并观察它的 UIA 树。
优先使用 list-apps/open/observe；只有 UIA 不足时再退回 OCR 或坐标。
```

macOS 上把 `$tactile-windows` 换成 `$tactile-macos`，选择 Calculator、TextEdit 或 Safari 这类简单应用验证即可。

### 可选 API 变量

只有在直接使用 skill 内置的 LLM workflow 命令时才需要这些变量；Codex 也可以用当前会话自己的模型一步步驱动 skill。
...
如果你的机器上 `python` 命令不是目标 Python，可以设置 `TACTILE_WINDOWS_PYTHON`。
